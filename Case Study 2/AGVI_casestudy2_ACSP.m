...
%% Missing points
% no_missing_points = 20;
% ind       = randsample(size(YT,2),no_missing_points);
% YT(:,ind) = nan;
% dat = load('AGVI_M_CS1_Data05.mat');
% YT  = dat.YT;
% figure;
% plot(YT(1,:))
...
%  Plotting Variances
    ...
    %     m      = logical(triu(mL_mat));
        ...
        P_P(:,:,t) = (P_P(:,:,t)+P_P(:,:,t)')/2;
        if any(eig(P_P(:,:,t))<0)  % eigen
            P_P(:,:,t)  = multiagvi.makePSD(P_P(:,:,t));
        end
        V_W(:,t)                  = diag(P_P(:,:,t));
    end
    ...
    mu          = [sW';sW_cov];
    s           = sqrt(V_W(:,end));
    t_statistic(p,:) = (x_bar' - mu')./(s');
    
    % Testing the null hypothesis
    
    t  = 1:length(EX);
%     l_var   = min(5,n_x);%min(10,n_x)
%     ind_var = randperm(size(sW,2),l_var);
%     figure;
    for i=1:5%ind_var
        figure;
%         subplot(5,2,i)
...
%     l_cov = min(10,size(sW_cov,1));
%     ind_cov = randperm(size(sW_cov,1),l_cov);
%     figure;
    for i=1:10%,ind_cov 
        figure;
%         subplot(5,2,i)
        ...
        hold off
        xlabel('$t$','Interpreter','latex')
        ylabel(['$\sigma_{W}' num2str(i) '$'],'Interpreter','latex')
%         ylim([-2,2])
        title('covariance')
    end
end
...
avg_t_statistic = mean(t_statistic);
ind_t       = avg_t_statistic(abs(avg_t_statistic) > t_critical);
if ~isempty(ind_t)
